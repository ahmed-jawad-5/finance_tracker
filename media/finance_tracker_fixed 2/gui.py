...
class FinanceTrackerApp:
    ...
    def toggle_theme(self):
        ...
        # Update budget tree colors if needed
        if hasattr(self, 'budget_tree'):
            if self.dark_theme.get():
                self.budget_tree.tag_configure('over', foreground='salmon')
                self.budget_tree.tag_configure('under', foreground='lightgreen')
            else:
                self.budget_tree.tag_configure('over', foreground='red')
                self.budget_tree.tag_configure('under', foreground='green')
    ...
    def setup_add_transaction_tab(self):
        ...
        # Fix the call to get_categories to pass the user_id
        self.category_entry['values'] = [""] + (self.db.get_categories(self.user_id) or [])
        ...
        self.filter_category['values'] = ["All"] + (self.db.get_categories(self.user_id) or [])

    # In add_transaction method (when updating category values):
        self.category_entry['values'] = [""] + (self.db.get_categories(self.user_id) or [])
        self.filter_category['values'] = ["All"] + (self.db.get_categories(self.user_id) or [])
        self.budget_category['values'] = self.db.get_categories(self.user_id) or []
    ...
    def setup_view_transactions_tab(self):
        ...
        # Initialize with empty list if no categories yet
        self.filter_category['values'] = ["All"] + (self.db.get_categories() or [])
        ...
        # Set up tag configurations for the treeview
        if self.dark_theme.get():
            self.tree.tag_configure('income', foreground='lightgreen')
            self.tree.tag_configure('expense', foreground='salmon')
        else:
    ...
    def setup_budget_tab(self):
        ...
        self.budget_category = ttk.Combobox(entry_frame, width=15)
        # Initialize with empty list if no categories yet
        self.budget_category['values'] = self.db.get_categories() or []
        ...
        # Pack the budget tree and scrollbar properly
        self.budget_tree.pack(fill=tk.BOTH, expand=True, side=tk.LEFT)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        # Set up tag configurations for the budget tree
        self.budget_tree.tag_configure('over', foreground='red')
        self.budget_tree.tag_configure('under', foreground='green')
    ...
    def delete_budget(self):
        ...
        if self.db.delete_budget(category):
            ...
            self.load_budgets()
            self.check_alerts()  # Update alerts after deletion
    ...
    def edit_transaction(self, event):
        ...
        edit_category['values'] = [""] + (self.db.get_categories() or [])
    ...
    def add_transaction(self):
        try:
            ...
            # Make sure to pass self.user_id as the first parameter
            if self.db.add_transaction(self.user_id, date, description, amount, category, transaction_type):
                messagebox.showinfo("Success", "Transaction added successfully!")
                self.desc_entry.delete(0, tk.END)
                self.amount_entry.delete(0, tk.END)
                
                # Refresh the category lists - ensure user_id is passed
                categories = self.db.get_categories(self.user_id) or []
    ...
    def set_budget(self):
        try:
            ...
            if self.db.add_budget(category, amount):
                ...
                self.load_budgets()
                self.check_alerts()  # Check alerts after setting budget
    ...
    def load_transactions(self):
        try:
            ...
            # Get transactions from database - pass user_id as first parameter
            transactions = self.db.get_transactions(self.user_id, start_date, end_date, category, 
                                            transaction_type.lower() if transaction_type and transaction_type != "All" else None, 
                                            search_keyword)
            ...
            for t in transactions:
                ...
                # Set tag based on transaction type for coloring
                tag = ttype.lower()
                
                # Add to tree
                self.tree.insert('', 'end', values=(id, date.strftime('%Y-%m-%d'), description, 
                                            amount_display, category or "", ttype.capitalize()), tags=(tag,))
