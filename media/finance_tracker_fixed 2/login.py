...
class window:
    ...
    def login(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(f"USE {self.db_name}")
            cursor.execute("SELECT id, first_name FROM users WHERE email = %s AND password = %s", (username, password))
            ...
            if user_data:
                ...
                messagebox.showinfo("Success", f"Welcome {first_name}! Login complete")
                self.root.destroy()
                self.launch_finance_tracker(user_id)
            else:
    ...
    def launch_finance_tracker(self, user_id):
        try:
            ...
            root = tk.Tk()
            app = FinanceTrackerApp(root, user_id)
            root.protocol("WM_DELETE_WINDOW", app.on_closing)
            root.mainloop()
        except ImportError as e:
            messagebox.showerror("Error", f"Could not launch Finance Tracker: {str(e)}")
